...
async function main() {
  ...
  if (dryRun) {
    ...
    console.log(HANDOVER_AUTO_COMPLETION_MIGRATION_SQL.trim());
    return;
  }
  ...
  try {
    ...
    await client.query(HANDOVER_AUTO_COMPLETION_MIGRATION_SQL);
    await client.query("commit");
    console.log(`Migrasi auto-finalisasi serah-terima berhasil diterapkan ke ${target}.`);
  } catch (error) {
