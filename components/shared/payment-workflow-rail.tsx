...
export function PaymentWorkflowRail({
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-x-0 top-0 h-1 bg-[linear-gradient(90deg,#0a6a49_0%,#20b96b_58%,#d7ad2f_100%)]" />
      ...
      <div className="relative space-y-6">
        ...
        <div className="relative">
          <div className="absolute left-[calc(100%/6)] right-[calc(100%/6)] top-[2.95rem] hidden h-1.5 overflow-hidden rounded-full bg-[#e3e8df] md:block">
            <div
              className="payment-flow-progress h-full rounded-full bg-[linear-gradient(90deg,#22b95f_0%,#20b96b_58%,#d7ad2f_100%)] transition-[width] duration-700 ease-out"
  ...
  return (
    <div className="relative text-center">
      <div className="relative mx-auto grid size-24 place-items-center md:size-[6.25rem]">
        {active ? (
          <>
            <span className="status-pulse absolute inset-3 rounded-full bg-[#d7ad2f]/30" />
            <span className="absolute inset-1 rounded-full border border-[#d7ad2f]/45" />
          </>
        ) : null}
        <span
          className={cn(
            ...
            active && "border-[#d7ad2f] text-[#0a6a49] shadow-[0_18px_38px_-22px_rgba(215,173,47,0.78)]",
        ...
        {active ? (
          <span className="absolute -bottom-1 z-20 rounded-full bg-[#ffd45a] px-3 py-1 text-[0.62rem] font-black uppercase tracking-[0.14em] text-[#3f3002] shadow-[0_10px_20px_-14px_rgba(85,60,0,0.8)]">
            Berjalan
          </span>
        ) : null}
      </div>
      ...
      <div className={cn("mx-auto mt-4", compact ? "max-w-[13rem]" : "max-w-[15rem]")}>
        ...
        {step.meta ? (
          <p className="mt-1 text-xs font-semibold uppercase tracking-[0.14em] text-[#0a6a49]/70">
            {step.meta}
          </p>
        ) : null}
        <p className="mt-2 text-xs leading-5 text-black/48">{step.detail}</p>
      </div>
      ...
      <div className="mx-auto mt-5 h-8 w-1 rounded-full bg-[#e3e8df] md:hidden">
        <div
          className={cn(
            ...
            completed ? "h-8 bg-[#22b95f]" : active ? "h-4 bg-[#d7ad2f]" : "h-0"
          )}
        />
      </div>
    </div>
