...
function buildRedemptionReference(
  ...
  const normalizedCode =
    normalizeToken(itemCode) || normalizeToken(itemId.slice(0, 8));
...
export function AdminRedeemForm({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    toast({
      ...
      description:
        "Sistem sedang menutup alur barang ini sebagai barang yang ditebus nasabah.",
      variant: "info",
      scope: "admin-unit",
      duration: 2600,
    });
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result?.message ?? "Penebusan belum berhasil diproses.",
        );
      }
      ...
      toast({
        ...
        description:
          "Barang keluar dari alur penjualan dan tercatat sebagai riwayat tebus.",
        variant: "success",
        scope: "admin-unit",
      });
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Coba ulangi setelah memeriksa data transaksi penebusan.",
        variant: "error",
        scope: "admin-unit",
        duration: 5600,
      });
            ...
            <div className="rounded-[1.15rem] border border-[#f5c88a] bg-[#fff7ed] px-4 py-2.5 text-[#c46d11] shadow-[inset_0_1px_0_rgba(255,255,255,0.65)] sm:px-5">
              <div className="flex items-start gap-3">
                ...
                <p className="text-[0.86rem] font-medium leading-6">
                  Penebusan akan menghapus aset dari daftar jaminan aktif secara
                  permanen.
                </p>
              </div>
            </div>
                ...
                <div className="rounded-[1.15rem] border border-slate-200/70 bg-[#f8fafc] p-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.72)]">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 space-y-1">
                      ...
                      <p className="text-[0.92rem] font-medium text-slate-500">
                        {itemCode}
                      </p>
